...
for t=1:n_test    
    ...
    for l=1:length(lambda_vals)
        ...
        if is_member('RandomSearch', implemented_algorithms)
            ...
            iSave(sprintf(save_filename_template, 'RandomSearch', t, inputs_t.lambda), rnd, 'rnd', inputs_t, 'inputs_t');
        end
        
        if is_member('SimulatedAnnealing', implemented_algorithms)
            sa = simulated_annealing(objective, inputs_t);
            fprintf('SA - Runtime = %f\n', sum(sa.runTime));
            iSave(sprintf(save_filename_template, 'SimulatedAnnealing', t, inputs_t.lambda), sa, 'sa', inputs_t, 'inputs_t');
        end
        
        if is_member('ExpectedImprovement', implemented_algorithms)
            bo = bayes_opt(objective, inputs_t);
            fprintf('BO - Runtime = %f\n', sum(bo.runTime));
            iSave(sprintf(save_filename_template, 'ExpectedImprovement', t, inputs_t.lambda), bo, 'bo', inputs_t, 'inputs_t');
        end
        
        if is_member('ObliviousLocalSearch', implemented_algorithms)
            ols = local_search(objective, inputs_t);
            fprintf('OLS - Runtime = %f\n', sum(ols.runTime));
            iSave(sprintf(save_filename_template, 'ObliviousLocalSearch', t, inputs_t.lambda), ols, 'ols', inputs_t, 'inputs_t');
        end
        
        if is_member('SequentialMonteCarlo', implemented_algorithms)
            smc = binary_smc(objective, inputs_t);
            fprintf('SMC - Runtime = %f\n', sum(smc.runTime));
            iSave(sprintf(save_filename_template, 'SequentialMonteCarlo', t, inputs_t.lambda), smc, 'smc', inputs_t, 'inputs_t');
        end
        
        if is_member('SMAC', implemented_algorithms)
            smac{l} = run_smac(objective, inputs_t);
            fprintf('SMAC - Runtime = %f\n', sum(smac.runTime));
            iSave(sprintf(save_filename_template, 'SMAC', t, inputs_t.lambda), smac, 'smac', inputs_t, 'inputs_t');
        end
...
%             iSave(sprintf(save_filename_template, 'BOCSorder1SA', t, inputs_t.lambda), bayes_SA1, 'bayes_SA1', inputs_t, 'inputs_t');
%         end

        if is_member('BOCS-SA', implemented_algorithms)
            bayes_SA2 = BOCS(inputs_t.model, penalty, inputs_t, 2, 'SA');
            fprintf('Bayes.SA2 - Runtime = %f\n', sum(bayes_SA2.runTime));
            iSave(sprintf(save_filename_template, 'BOCSorder2SA', t, inputs_t.lambda), bayes_SA2, 'bayes_SA2', inputs_t, 'inputs_t');
        end

%         if is_member('BOCS-SA', implemented_algorithms)
%             bayes_SA3 = BOCS(inputs_t.model, penalty, inputs_t, 3, 'SA');
%             fprintf('Bayes.SA3 - Runtime = %f\n', sum(bayes_SA3.runTime));
%             iSave(sprintf(save_filename_template, 'BOCSorder3SA', t, inputs_t.lambda), bayes_SA3, 'bayes_SA3', inputs_t, 'inputs_t');
%         end

        if is_member('BOCS-SA', implemented_algorithms)
            bayes_sdp = BOCS(inputs_t.model, penalty, inputs_t, 2, 'sdp');
            fprintf('Bayes.SDP - Runtime = %f\n', sum(bayes_sdp.runTime));
            iSave(sprintf(save_filename_template, 'BOCSorder2SDP', t, inputs_t.lambda), bayes_sdp, 'bayes_sdp', inputs_t, 'inputs_t');
        end
...
%             iSave(sprintf(save_filename_template, 'MLEorder1SA', t, inputs_t.lambda), mle_SA1, 'mle_SA1', inputs_t, 'inputs_t');
%         end

        if is_member('BOCS-MLE', implemented_algorithms)
            mle_SA2 = BOCS(inputs_t.model, penalty, inputs_t, 2, 'SA');
            fprintf('MLE.SA2 - Runtime = %f\n', sum(mle_SA2.runTime));
            iSave(sprintf(save_filename_template, 'MLEorder2SA', t, inputs_t.lambda), mle_SA2, 'mle_SA2', inputs_t, 'inputs_t');
        end

%         if is_member('BOCS-MLE', implemented_algorithms)
%             mle_SA3 = BOCS(inputs_t.model, penalty, inputs_t, 3, 'SA');
%             fprintf('MLE.SA3 - Runtime = %f\n', sum(mle_SA3.runTime));
%             iSave(sprintf(save_filename_template, 'MLEorder3SA', t, inputs_t.lambda), mle_SA3, 'mle_SA3', inputs_t, 'inputs_t');
%         end

        if is_member('BOCS-MLE', implemented_algorithms)
            mle_sdp = BOCS(inputs_t.model, penalty, inputs_t, 2, 'sdp');
            fprintf('MLE.SDP - Runtime = %f\n', sum(mle_sdp.runTime));
            iSave(sprintf(save_filename_template, 'MLEorder2SDP', t, inputs_t.lambda), mle_sdp, 'mle_sdp', inputs_t, 'inputs_t');
        end
...
%             iSave(sprintf(save_filename_template, 'HorseShoeorder1SA', t, inputs_t.lambda), hs_SA1, 'hs_SA1', inputs_t, 'inputs_t');
%         end

        if is_member('BOCS-HS', implemented_algorithms)
            hs_SA2 = BOCS(inputs_t.model, penalty, inputs_t, 2, 'SA');
            fprintf('HS.SA2 - Runtime = %f\n', sum(hs_SA2.runTime));
            iSave(sprintf(save_filename_template, 'HorseShoeorder2SA', t, inputs_t.lambda), hs_SA2, 'hs_SA2', inputs_t, 'inputs_t');
        end

%         if is_member('BOCS-HS', implemented_algorithms)
%             hs_SA3 = BOCS(inputs_t.model, penalty, inputs_t, 3, 'SA');
%             fprintf('HS.SA3 - Runtime = %f\n', sum(hs_SA3.runTime));
%             iSave(sprintf(save_filename_template, 'HorseShoeorder3SA', t, inputs_t.lambda), hs_SA3, 'hs_SA3', inputs_t, 'inputs_t');
%         end

        if is_member('BOCS-HS', implemented_algorithms)
            hs_SDP = BOCS(inputs_t.model, penalty, inputs_t, 2, 'sdp');
            fprintf('HS.SDP - Runtime = %f\n', sum(hs_SDP.runTime));
            iSave(sprintf(save_filename_template, 'HorseShoeorder2SDP', t, inputs_t.lambda), hs_SDP, 'hs_SDP', inputs_t, 'inputs_t');
        end
