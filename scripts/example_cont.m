...
seed = randi(10000,1);
[initialX, Lambda, Gamma]=rand_contamination_components(inputs.n_vars, mcSamps, seed);

% Save objective function and regularization term
inputs.model    = @(x) contamination_prob(x, mcSamps, initialX, Lambda, Gamma);
...
smc = binary_smc(objective, inputs);
