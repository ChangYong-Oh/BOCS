...
from compute_derivs import comp_partial_derivatives

import pickle
import matplotlib.pyplot as plt
import matplotlib.mlab as mlab
from velcoupling import velcoupling
import copy
import os.path
...
def input_cov(rand_inputs):
    ...
    for input in rand_inputs:
        ...
        input_idx = rand_inputs.keys().index(input)
...
def decoupled_vars(N, loads_mat, mesh_mat, dec_loads_vect, dec_mesh_vect):
    ...
    if (np.max(loads_mat)+1) != len(dec_loads_vect):
        sys.exit('Specified model is of incorrect size')
...
def dec_mat(N, n_subdom):
    ...
    if np.mod(pts_per_side, n_subdom) != 0:
        sys.exit('Number of Subdomains doesn''t divide mesh size!')
...
if __name__ == '__main__':
    ...
    # Setup model_params    
    model_params = {}
    model_params['rand_inputs']  = rand_inputs
    model_params['fixed_inputs'] = fixed_inputs
    ...
    model_mesh  = dec_models['model_mesh'][0]
    ...
    models  = np.concatenate((model_loads, model_mesh))
    sio.savemat('results.mat', {'models':models, 'kl_vect': np.array([kl_value])})
